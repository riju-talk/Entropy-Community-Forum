  ...
  return (
    <aside className="w-64 border-r border-white/5 bg-background/60 backdrop-blur-xl flex flex-col h-full px-6 py-8 relative overflow-hidden group/sidebar">
      ...
      <div className="space-y-6">
        <h2 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest pl-4">
          Explore
        </h2>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <div
                  className={cn(
                    "flex items-center gap-3 w-full px-4 py-3 rounded-2xl transition-all duration-300 relative overflow-hidden",
                    isActive
                ...
                >
                  ...
                  {item.comingSoon && (
                    <Badge variant="outline" className="ml-auto text-[9px] bg-purple-500/10 text-purple-400 border-purple-500/20 uppercase tracking-widest">
                      Soon
                    </Badge>
                  )}
                </div>
          ...
          {communities.length > 0 ? (
            communities.slice(0, 5).map((community: any) => (
              <Link
                ...
                className="block group"
              >
                <div className="flex items-center gap-3 w-full px-4 py-2 rounded-xl text-muted-foreground hover:text-foreground hover:bg-white/5 transition-all relative">
                  <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-[10px] font-bold text-cyan-400 border border-white/5 group-hover:border-cyan-500/30 group-hover:bg-cyan-500/10 transition-all">
                    {community.name?.[0]?.toUpperCase()}
                  </div>
                  <span className="text-sm font-medium truncate group-hover:text-cyan-200 transition-colors">{community.name}</span>
                </div>
              </Link>
            ))
          ) : (
            <div className="px-4 text-xs text-muted-foreground/50 italic">No communities yet</div>
          )}
        </div>
      </div>
      ...
      <div className="mt-auto pt-6">
        {isAuthenticated ? (
          <Card className="bg-gradient-to-br from-background to-white/5 border-white/10 overflow-hidden relative group hover:border-cyan-500/30 transition-all duration-500">
            <CardContent className="p-3 space-y-3 relative z-10">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 bg-gradient-to-tr from-cyan-500 to-blue-600 rounded-xl flex items-center justify-center text-white text-sm font-bold shadow-lg shadow-cyan-500/20">
                  {userInitial}
                </div>
              ...
              <Button
                variant="ghost"
                size="sm"
                className="w-full h-8 text-[10px] uppercase tracking-wider hover:bg-red-500/10 hover:text-red-400 transition-colors"
          ...
          <Button
            variant="default" // This will use the new primary color
            size="sm"
            className="w-full h-11 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.5)] transition-all duration-300 rounded-xl"
